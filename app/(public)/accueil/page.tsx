...
export default function HomePage() {
  ...
  const toneStyles = {
    primary: { bg: "color-mix(in srgb, var(--theme-primary) 15%, transparent)", text: "var(--theme-primary)" },
    secondary: { bg: "color-mix(in srgb, var(--theme-secondary) 15%, transparent)", text: "var(--theme-secondary)" },
    accent: { bg: "color-mix(in srgb, var(--theme-accent) 15%, transparent)", text: "var(--theme-accent)" },
    danger: { bg: "color-mix(in srgb, var(--theme-danger) 15%, transparent)", text: "var(--theme-danger)" },
  } as const;
    ...
    <div className="space-y-10">
      ...
      <VisibilityWrapper componentId="home.features">
        ...
        {content.features.map((item, index) => {
          ...
          const Icon = [GlassWater, Music, ShieldAlert][index] || Sparkles;
          return (
          <div
            key={index}
          ...
          >
            ...
            <div className="absolute -right-12 -top-12 w-32 h-32 rounded-full blur-2xl group-hover:scale-150 transition-transform duration-500 pointer-events-none" style={{ backgroundColor: tone.bg }} />

            {/* En-tête : Icône + Badge */}
            <div className="flex items-center justify-between">
              <div className="w-14 h-14 text-slate-900 rounded-2xl flex items-center justify-center shadow-lg shadow-slate-900/10 group-hover:scale-110 transition-transform duration-300" style={{ backgroundColor: tone.text }}>
                <Icon className="w-7 h-7" />
              </div>
              <span className="text-[11px] font-bold uppercase tracking-wider px-3 py-1 rounded-full" style={{ backgroundColor: tone.bg, color: tone.text }}>
                {item.badge}
              </span>
            </div>
            ...
            <div className="space-y-2">
              <h3 className="text-xl font-extrabold tracking-tight transition-colors" style={{ color: "var(--theme-textPrimary)" }}>
                {item.title}
              </h3>
              <p className="text-sm leading-relaxed font-medium" style={{ color: "var(--theme-textSecondary)" }}>
                {item.desc}
              </p>
            </div>
          </div>
        ...
        <section className="group relative border-3 p-5 sm:p-6 rounded-2xl flex flex-col md:flex-row items-start md:items-center justify-between gap-6 overflow-hidden transition-all duration-300" style={{ backgroundColor: 'var(--theme-accent)', borderColor: 'var(--theme-primary)', boxShadow: `6px 6px 0px 0px var(--theme-primary)` }}>
          ...
          <div className="space-y-2 relative z-10 max-w-2xl">
            ...
            <h2 className="text-xl sm:text-2xl font-black uppercase tracking-tight leading-snug" style={{ color: 'var(--theme-primary)' }}>
              {regulation.title} : <span className="underline decoration-4 underline-offset-4" style={{ textDecorationColor: 'var(--theme-danger)' }}>{regulation.highlight}</span>
            </h2>

            <p className="text-xs sm:text-sm font-medium leading-relaxed" style={{ color: `var(--theme-primary)90` }}>
              <strong className="font-black" style={{ color: 'var(--theme-primary)' }}>Rappel :</strong> {regulation.description}
            </p>
          </div>
          ...
          >
            <span>{regulation.buttonLabel}</span>
